...
describe('speech suggestions', () => {
	...
	it('creates one base session, clones it for each generation, and destroys only the clones', async () => {
		const fake = fakeApi();
		...
		expect(firstClone.prompt).toHaveBeenCalledWith(expect.stringContaining('アルパカ'), expect.objectContaining({ responseConstraint: expect.any(Object) }));
	...
	it('clears a broken base after clone failure and preserves it for clone/prompt aborts', async () => {
		...
		abortClone.prompt.mockRejectedValue(new DOMException('cancelled', 'AbortError'));
