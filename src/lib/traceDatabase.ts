...
export type TraceRootReadRecord = Readonly<{
	channelId: string;
	eventId: string;
	read: true;
}>;
...
export async function openTraceDatabase() {
	return openDB<TraceDatabase>(TRACE_DATABASE_NAME, TRACE_DATABASE_VERSION, {
		upgrade(db) {
			...
			if (!db.objectStoreNames.contains(TRACE_ROOT_READ_STORE)) {
				db.createObjectStore(TRACE_ROOT_READ_STORE, { keyPath: ['channelId', 'eventId'] });
			}
