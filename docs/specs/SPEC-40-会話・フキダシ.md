...
一度だけ行い、表示後のresize・端末回転でユーザーのshow/hide状態を上書きしない。Chatterの可視toggleはActionDock内の単一buttonとし、表示テキストは`Chatter`、open/closed状態をaccessibility stateで判別可能にする。timeline内容をlocalStorage、IndexedDB等へ保存しない。
...
Prompt APIを実際に利用できる対応環境では、現在のキャラクター、発言タイプ、既存の会話状況を
参考に、短い日本語の発言候補を任意で生成できる。これは候補を提示する補助機能であり、AIは
自動投稿、自動返信、自律会話を行わない。

各候補には2つの明示的な操作を持つ。候補本文をクリックした場合は、その候補をそのまま送信する。
候補右側の「追加」をクリックした場合は、候補を通常のComposer本文へ設定し、ユーザーが通常の本文と
同じように編集してから明示的な投稿操作を行う。いずれの場合も生成だけでは投稿せず、AI自身が自律的に
投稿・返信を開始しない。直接送信も必ずユーザーの候補本文クリックを起点とする。

候補panelは右上の閉じる操作で明示的に閉じられる。

候補生成開始時と「追加」操作時には本文が空であることを確認し、既存のdraftを候補で上書きしない。
候補本文からの直接送信は親クライアントが所有する明示的な送信controlとして扱い、通常のComposer投稿と
同じ発言タイプ・Relay readiness・reply target検証・kind 42 / kind 1111 publish経路を利用する。
Trace reply mode中の直接送信は、そのクリック時点のroot / targetをsnapshotし、publication直前に同じ
reply contextであることを再確認する。送信に成功した場合は候補UIを閉じ、失敗またはabortの場合は候補を
失わず再試行できる状態を保つ。

対応していない環境では発言候補UIを表示せず、通常のComposerと会話機能へ影響を与えない。この
機能はブラウザ内のオンデバイスPrompt APIだけを利用し、cloud LLM、外部API、専用backend等の
代替経路を前提としない。候補生成のためだけに新しいRelay readやsubscriptionを開始しない。
