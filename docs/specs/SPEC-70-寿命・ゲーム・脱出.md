...
PC表示ではコンパクトパネルと結果詳細パネルの本文を14ptとする。参加受付の締切・カウントダウンと展開したルール本文も14ptに含める。見出し・操作ボタン・補助表記は読みやすさとコンパクトさを保つよう調整する。スマートフォンは既存のレスポンシブな文字サイズを維持し、パネルと詳細をviewport内でスクロールして確認できるようにする。
