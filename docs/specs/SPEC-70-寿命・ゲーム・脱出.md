...
フィールド上に固定施設として `繕い端末` を配置する。端末は1つの論理cellを占有し、専用clientのlocal playerはそのcellへ侵入できない。端末の周囲8cellから操作できる。プレイヤーは端末の近くまで実際に移動しないと、繕いの開始・完了処理をできない。端末では次を行う。
...
繕いは最大処理時間まで非同期に進行し、最大処理時間に達するとそれ以上成果を増やさず停止する。完了後は繕い端末へ戻り、成果を確認してから次の繕いを開始する。具体的な端末座標、placeholderの見た目、将来の専用assetは本仕様で固定しない。
