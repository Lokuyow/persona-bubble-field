...
公式イベントパネルはゲーム進行中に基本情報をコンパクトに表示し、各参加者には自身のグループの最新結果と本人の報酬・ペナルティを優先して示す。未参加者にはグループ結果を表示しない。前ラウンドの結果を表示する場合は過去の結果と明記する。有効な選択、成否、共通の報酬・ペナルティ、本人の結果の詳細は、独立した非モーダルパネルを明示的に開いて確認できる。詳細パネルは個別にスクロールでき、フィールドの通常操作を詳細パネル外で妨げない。ラウンドが切り替わると詳細を閉じるが、表示可能な結果は再度開いて参照できる。
...
PC表示ではコンパクトパネルと結果詳細パネルの本文を14ptとし、見出し・残り時間・操作ボタン・補助表記は読みやすさとコンパクトさを保つよう調整する。スマートフォンは既存のレスポンシブな文字サイズを維持し、パネルと詳細をviewport内でスクロールして確認できるようにする。

参加受付中に自分が参加し、受付からラウンド1の相談フェーズへ移った時点で、確定した参加者snapshotに自分を含む3人以上のグループがある場合に、短い開始効果音を同じ開催につき一度だけ再生する。人数不足の中止、未参加、reloadや再接続後にゲーム中の状態を復元した場合は再生しない。再生は既存のWeb Audio効果音、音量設定、ミュート、ブラウザの音声再生制限に従い、再生できなくてもゲーム進行を妨げない。
