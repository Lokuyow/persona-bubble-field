...
参加受付中はゲーム内からルール説明を開ける。説明には、3〜6人・全3ラウンド、各ラウンドの時間、二つの選択肢、人数ごとの必要な維持人数、報酬、維持人数不足時の死亡、綻びでの脱出は通常の1000pt clearではないこと、選択が結果発表まで公開されないことを、一般ユーザー向けの自然な短い文で示す。抜け穴への参加を確定する直前には、`3〜6人 / 全3ラウンド` と、維持人数不足時に脱出を選んだ者が死亡する旨を表示し、ルール確認または参加を選べるようにする。説明と確認の全文および操作ボタンはスマートフォンのviewport内でスクロールして到達できる。音声、SE、追加演出は綻びの仕様に含めない。
