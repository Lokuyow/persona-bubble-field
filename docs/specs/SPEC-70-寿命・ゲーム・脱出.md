...
公式イベントパネルはゲーム進行中に基本情報をコンパクトに表示し、各参加者には自身のグループの最新結果から、有効な本人の選択とグループ判定に基づく「協力成功」「協力失敗」「抜け駆け成功」「抜け駆け失敗」等の本人結果と本人の報酬・ペナルティを示す。有効な本人の選択が確認できない場合は「本人の選択未確認」と表示し、結果を推測しない。未参加者にはグループ結果を表示しない。前ラウンドの結果を表示する場合は過去の結果と明記する。有効な選択、グループ全体の判定、共通の報酬・ペナルティ、本人の結果の詳細は、独立した非モーダルパネルを明示的に開いて確認できる。詳細パネルは個別にスクロールでき、フィールドの通常操作を詳細パネル外で妨げない。ラウンドが切り替わると詳細を閉じるが、表示可能な結果は再度開いて参照できる。
