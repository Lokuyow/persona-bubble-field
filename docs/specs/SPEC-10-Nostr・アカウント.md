...
脱出条件を満たす前は、通常のUI、ブラウザストレージの単純な閲覧、容易なコピー操作だけで、一般ユーザーが秘密鍵を簡単に取得できない状態にする。保存時には暗号化、難読化、その他のクライアント側処理を用い、平文の秘密鍵がそのまま容易に読み取れる状態を避ける。
...
ゲーム仕様で定める1000pt到達後の脱出選択で `ソトへ出る` を選択したときに、一般Nostrへ移行するための秘密鍵exportを提供する。1000pt到達時に自動的に脱出させず、`ハコに残る` を選択する場合の扱いは [`SPEC-70-寿命・ゲーム・脱出.md`](./SPEC-70-寿命・ゲーム・脱出.md) を正とする。
