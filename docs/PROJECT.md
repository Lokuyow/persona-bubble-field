...
- 同じ操作画面ではアクセント塗りつぶしの主要操作を最大1つとし、Primary・Secondary・Tertiaryで視覚的な優先順位を示す。操作の意味は優先順位と分けて扱い、取消・辞退には控えめなローズ系、不可逆的・破壊的な操作には危険度に応じた赤系を使う。Primaryはブランドカラーの塗りつぶし、Secondaryは淡い青みのグレー系の塗りつぶしと明確な境界、Tertiaryは中立色の背景と境界を持ち、通常状態で操作可能と分かるようにする。無効なPrimaryは鮮やかな塗りつぶしを残さない。通常・hover・active・disabled・focus-visibleの色と境界は共通ActionButtonのデザイントークンで管理し、コンパクトなアイコン操作も共通トークンを使って通常状態の輪郭を保つ。色だけで意味を伝えず既存のラベルとアイコンを維持する。閉じる・解除などの補助操作を主要操作として強調せず、選択肢や状態表示の意味は維持する。
