...
- Primary・Secondary・Tertiaryは操作の視覚的な優先順位を示し、Cancel・Dangerなどのintentと有効・無効状態はそれぞれ独立して扱う。Primaryはブランドカラーの塗りつぶし、Secondaryは淡い青みのグレー系の塗りつぶしと明確な境界、Tertiaryは中立色の背景と境界を持ち、通常状態で操作可能と分かるようにする。Primaryの個数は画面全体で制限せず、独立したカードや繰り返し項目では判断単位ごとに優先順位を決め、並列で同格の主要操作には複数のPrimaryを使用してよい。ただしカードが別であるだけで全操作をPrimaryにせず、同一判断単位内の優先順位は区別する。取消・辞退には控えめなローズ系、不可逆的・破壊的な操作には危険度に応じた赤系を使い、両者を混同しない。無効なPrimaryには彩度を落としたブランド系の専用色を使い、有効な操作と無効な操作を視覚的に識別できるようにする。通常・hover・active・disabled・focus-visibleの色と境界は共通ActionButtonのデザイントークンで管理し、コンパクトなアイコン操作も共通トークンを使って通常状態の輪郭を保つ。色だけで意味を伝えず既存のラベルとアイコンを維持する。閉じる・解除などの補助操作を主要操作として強調せず、選択肢や状態表示の意味は維持する。
- アプリ管理下のダイアログ・パネルの閉じる操作は、Tabler X SVGのみを使うTertiaryアイコンボタンに統一する。アイコンは24×24px、操作領域は44×44pxとし、アクセシブル名を付け、原則ヘッダー右端に配置する。スクロール領域から独立して閉じる操作へ到達できるようにし、キャンセルなど意味の異なる操作は閉じるボタンへ置き換えない。
